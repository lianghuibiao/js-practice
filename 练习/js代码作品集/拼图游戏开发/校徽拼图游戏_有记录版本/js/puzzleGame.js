...
{ 
    ...
    // window.localStorage = name + "="+ escape (value) + ";expires=" + exp.toGMTString(); 
    // alert(document.cookiea)
...
function getShortestTime(level)
{
    var shortestTime = getCookie("shortestTime_" + level)
    // console.log(shortestTime)
    return shortestTime || 99999;
}
...
function puzzleGame(contentArea, imageSrc, level) {
    ...
    //绑定全局事件
    this.creatEvent();
    $("#shortestTime").html(getShortestTime(defaultLevel.row))
}
...
puzzleGame.prototype = {
    ...
    startGame: function(successCallback) {
        ...
        this.timeHandle = setInterval(function () {
            $("#costTime").html(parseFloat((new Date() - beginTime) / 1000.0).toFixed(1))
        }, 100)
        // alert(getShortestTime(this.level.row))
        $("#shortestTime").html(getShortestTime(this.level.row))
    },
    ...
    resetGame: function(row, low) {
        ...
        // console.log(getShortestTime(this.level.row))
        $("#shortestTime").html(getShortestTime(this.level.row))
    },
    ...
    checkSuccess: function() {
        ...
        if (this.originalOrder.toString() == this.randomOrder.toString()) {
            clearInterval(this.timeHandle)
            // console.log("shortestTime_" + this.level.row)
            var usedTime = parseFloat((new Date() - this.beginTime) / 1000.0).toFixed(1)
            // console.log(Math.min(getShortestTime(this.level.row), usedTime))
            usedTime = Math.min(getShortestTime(this.level.row), usedTime)
            setCookie("shortestTime_" + this.level.row, "" + usedTime, "d3")
            $("#shortestTime").html(getShortestTime(this.level.row))
